...
export default function FreelanceStripeCheckoutModal({
  open,
  role,
  orderId,
  title,
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        const initRes = await fetch('/api/freelance/payment/stripe/create-intent', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ orderId, method }),
        });
        const initData = (await initRes.json()) as StripeInitResponse;
  ...
  async function handleConfirm() {
    ...
    try {
      setSubmitting(true);
      setError('');
      const returnUrl = `${window.location.origin}/${role}/freelance?payment=processing&order=${encodeURIComponent(orderId)}`;
    ...
    >
      ...
      >
        ...
        <div style={{ padding: 24 }}>
          ...
          >
            <div>
              <div style={{ color: COLORS.muted, fontSize: 12, fontWeight: 700 }}>Escrow total</div>
              <div
                style={{
                  color: COLORS.text,
                  fontSize: 18,
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: COLORS.muted }}>
              ...
              <span style={{ fontSize: 12 }}>
                After payment, the money stays in superadmin escrow until client approval.
              </span>
            </div>
            <button
            ...
            >
              {submitting ? (
                <>
                  <LoaderCircle size={16} className="freelance-stripe-loader" />
                  Redirecting...
                </>
              ) : (
                <>Pay BDT {amount}</>
        ...
        <style>{`
          .freelance-stripe-loader {
            animation: freelance-stripe-spin 0.9s linear infinite;
          }

          @keyframes freelance-stripe-spin {
            to {
